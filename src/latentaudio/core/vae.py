...
class Encoder(nn.Module):
    ...
    def __init__(self, waveform_length: int, latent_dim: int = LATENT_DIM):
        ...
        for out_ch, k, s in zip(
            ENCODER_CONV_CHANNELS, ENCODER_CONV_KERNEL_SIZES, ENCODER_CONV_STRIDES
        ):
            self.blocks.append(
                nn.Sequential(
...
class DecoderBlock(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, skip: Optional[torch.Tensor] = None, skip_prob: float = 0.0
    ) -> torch.Tensor:
        if self.skip_c > 0:
            if skip is None:
...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        bottleneck_len: int,
        bottleneck_ch: int,
        encoder_skip_shapes: List[Tuple[int, int]],
    ):
        super().__init__()
        self.latent_dim = latent_dim
        self.waveform_length = waveform_length
        ...
        for out_f in reversed(DECODER_FC_LAYERS):
            fc_layers.extend([nn.Linear(curr_in, out_f), nn.LeakyReLU(LEAKY_RELU_SLOPE)])
            curr_in = out_f
...
class UnconditionalVAE(nn.Module):
    ...
    def forward(
        ...
        reconstructed = self.decoder(
            z, skips=skips, skip_prob=skip_prob, use_generated_skips=use_generated_skips
        )
