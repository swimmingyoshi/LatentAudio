# simple_vae.py - COMPLETE FIXED VERSION - Prevents Posterior Collapse
...
class SimpleEncoder(nn.Module):
    ...
    def __init__(self, waveform_length: int, latent_dim: int = LATENT_DIM):
        ...
        logger.info(f"SimpleEncoder: {waveform_length} -> {latent_dim}D (conv_out={self.conv_out_len}, flat={flattened_size})")
...
class SimpleDecoder(nn.Module):
    ...
    def __init__(self, latent_dim: int, waveform_length: int, conv_out_len: int, skip_shapes: List[Tuple[int, int]]):
        super().__init__()
        ...
        logger.info(f"SimpleDecoder: {latent_dim}D -> {waveform_length} (with skip connections, bias=False)")
    
    def forward(self, z: torch.Tensor, skips: Optional[List[torch.Tensor]] = None, 
                use_skips: bool = True, use_generated_skips: bool = False) -> torch.Tensor:
        # Expand from latent
        h = self.fc(z)
        ...
        if use_generated_skips:
            ...
            curr_skips = skips[::-1] # Reverse encoder skips (shallow->deep) to decoder order (deep->shallow)
        else:
            curr_skips = []
...
class SimpleFastVAE(nn.Module):
    ...
    def __init__(self, latent_dim: int = LATENT_DIM, waveform_length: int = 44100):
        ...
        self.decoder = SimpleDecoder(latent_dim, waveform_length, self.encoder.conv_out_len, skip_shapes)
        ...
        logger.info("✓ Anti-collapse measures: MIN_LOGVAR=-6.0, separate μ/σ networks, logvar bias=-1.0")
    ...
    def decode(self, z: torch.Tensor, skips: Optional[List[torch.Tensor]] = None) -> torch.Tensor:
        ...
        return self.decoder(z, skips, use_skips=skips is not None, use_generated_skips=skips is None)
    ...
    def forward(self, x: torch.Tensor, skip_prob: Optional[float] = None,
        ...
        reconstructed = self.decoder(z, skips, use_skips=True, use_generated_skips=use_generated_skips)
