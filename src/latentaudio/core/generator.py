# generator.py - NO WARMUP VERSION
...
from ..types import (
    GeneratorConfig, TrainingConfig, GenerationConfig, TrainingMetrics,
    LatentVector, AudioArray, LatentPreset
)
...
from ..config import AUDIO_LIBS_AVAILABLE, LOG_AUDIO_INTERVAL, LOW_PASS_ORDER, DEFAULT_POSTPROCESS_LOW_PASS
...
class AdvancedAudioGenerator:
    ...
    def __init__(self, config: Optional[GeneratorConfig] = None):
        ...
        self.device = torch.device(config.device if config.device else 
        ...
        logger.info(f"AdvancedAudioGenerator initialized: {config.sample_rate}Hz, {config.duration}s, device={self.device}")
    
    def build_model(self, latent_dim: Optional[int] = None, skip_dropout_prob: float = 0.5, use_simple: bool = True) -> nn.Module:
    ...
    def train(
        ...
        self.training_logger.log_audio('Comparison/Original', fixed_sample, self.config.sample_rate, 0)
        ...
        if torch.isnan(X).any():
            ...
            X = X[valid_mask]
            logger.warning(f"Removed {(~valid_mask).sum()} corrupted samples. {len(X)} samples remaining.")
        ...
        try:
            ...
            for session_epoch in range(1, config.epochs + 1):
                ...
                epoch_loss, epoch_recon, epoch_kl, epoch_waveform, epoch_spectral, epoch_skip = 0, 0, 0, 0, 0, 0
                n_batches = 0
                ...
                for i, (batch_x,) in progress_bar:
                    ...
                    if hasattr(self, '_last_grad_overflow') and self._last_grad_overflow:
                        ...
                        logger.debug(f"Disabling AMP for batch {i} due to previous gradient overflow")
                    ...
                    with torch.amp.autocast('cuda', enabled=use_amp_this_batch):
                        ...
                        if reconstructed.abs().max() > 100 or mu.abs().max() > 50 or logvar.max() > 10:
                            logger.warning(
                        ...
                        loss_dict = compute_vae_loss(
                            reconstructed, batch_x, mu, logvar, 
                            current_beta, config, mrstft, 
                            model=self.model, z=z_sample, 
                    ...
                    for name, param in self.model.named_parameters():
                        if param.grad is not None and (torch.isnan(param.grad).any() or torch.isinf(param.grad).any()):
                            logger.error(f"Epoch {epoch}, Batch {i}: NaN/Inf gradient detected in {name}")
                            self._last_grad_overflow = True
                            break
                    
                    if self._last_grad_overflow:
                        logger.warning(f"Epoch {epoch}, Batch {i}: NaN/Inf gradient (norm=inf). Skipping step.")
                        optimizer.zero_grad(set_to_none=True)
                        scaler.update()
                    ...
                    if (i + 1) % config.gradient_accumulation_steps == 0:
                        ...
                        if torch.isnan(grad_norm) or torch.isinf(grad_norm):
                            logger.warning(f"Epoch {epoch}, Batch {i}: NaN/Inf gradient (norm={grad_norm}). Skipping.")
                ...
                if epoch % 50 == 0:
                    if self.model.check_for_nan():
                        logger.error(f"NaN detected in model parameters at epoch {epoch}! Resetting...")
                ...
                if epoch % (LOG_AUDIO_INTERVAL) == 0:
                    ...
                    with torch.no_grad():
                        ...
                        self.training_logger.log_audio('Reconstruction/Fixed_Skips', recon_fixed, self.config.sample_rate, epoch)
                        
                        recon_np = recon_fixed.detach().cpu().numpy().squeeze()
                        recon_img = plot_waveform_image(recon_np, self.config.sample_rate, f"Fixed Skip Recon (Epoch {epoch})")
                        self.training_logger.log_image('Plots/Reconstruction_Fixed', recon_img, epoch)
                        ...
                        self.training_logger.log_audio('Reconstruction/Pure_Latent', recon_pure, self.config.sample_rate, epoch)
                        
                        pure_np = recon_pure.detach().cpu().numpy().squeeze()
                        pure_img = plot_waveform_image(pure_np, self.config.sample_rate, f"Pure Latent Recon (Epoch {epoch})")
                        ...
                        self.training_logger.log_audio('Generation/Random', generated, self.config.sample_rate, epoch)
                        
                        gen_np = generated.detach().cpu().numpy().squeeze()
                        gen_img = plot_waveform_image(gen_np, self.config.sample_rate, f"Random Generation (Epoch {epoch})")
    ...
    def generate_from_latent(self, latent_vector: LatentVector, config: Optional[GenerationConfig] = None) -> AudioArray:
    ...
    def generate_random(self, n_samples: int = 1, config: Optional[GenerationConfig] = None) -> List[AudioArray]:
    ...
    def _postprocess(self, audio: AudioArray) -> AudioArray:
        ...
        sos = signal.butter(LOW_PASS_ORDER, DEFAULT_POSTPROCESS_LOW_PASS, 'lp', 
    ...
    def interpolate(self, z1: LatentVector, z2: LatentVector, n_steps: int = 10, method: str = 'linear') -> List[LatentVector]:
        return self.interpolator.interpolate(z1, z2, n_steps, method)
    
    def random_walk(self, start_vector: Optional[LatentVector] = None, n_steps: int = 8, 
                    step_size: float = 0.4, temperature: float = 1.0, 
                    momentum: float = 0.5, origin_pull: float = 0.1) -> List[LatentVector]:
        return self.walker.random_walk(start_vector, n_steps, step_size, temperature, momentum, origin_pull)
    ...
    def load_model(self, filepath: Union[str, Path]) -> Dict[str, Any]:
        ...
        if model_type == 'simple' or 'encoder.conv' in str(checkpoint_dict['model_state'].keys()):
            self.model = SimpleFastVAE(latent_dim=self.config.latent_dim, waveform_length=self.waveform_length).to(self.device)
        else:
            self.model = UnconditionalVAE(latent_dim=self.config.latent_dim, waveform_length=self.waveform_length).to(self.device)
        ...
        self.cached_latents = np.array(explorer_data['cached_latents']) if explorer_data.get('cached_latents') is not None else None
        ...
        if 'directions' in explorer_data:
            for name, d_data in explorer_data['directions'].items():
                try:
                    ...
                    self.attribute_explorer.discovered_directions[name] = DiscoveredDirection.from_dict(d_data)
                except:
                    pass
    ...
    def save_preset(self, name: str, vector: LatentVector, description: str = "") -> None:
        self.preset_manager.save_preset(LatentPreset(name=name, vector=vector, description=description))
    ...
    # --- Attribute Discovery ---
    def discover_direction(self, name: str, positive_samples: List[LatentVector], negative_samples: List[LatentVector]):
        return self.attribute_explorer.discover_direction(name, positive_samples, negative_samples)

    def apply_attribute(self, base_vector: LatentVector, direction_name: str, strength: float) -> LatentVector:
