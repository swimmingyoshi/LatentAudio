    ...
    def log_metrics(self, epoch: int, metrics: TrainingMetrics) -> None:
        ...
        if epoch % LOG_METRICS_INTERVAL == 0:
            ...
            self.writer.add_scalar(
                "Loss/Reconstruction_Combined", metrics.reconstruction_loss, epoch
            )
    ...
    def start_tensorboard(self, port: int = 6006, host: str = "localhost") -> None:
        ...
        def run_tensorboard():
            try:
                ...
                if os.name == "nt":
                    try:
                        find_port = subprocess.check_output(
                        ...
                        for line in find_port.strip().split("\n"):
                            if "LISTENING" in line:
                                ...
                                subprocess.run(
                                    ["taskkill", "/F", "/PID", pid, "/T"], capture_output=True
                                )
                        time.sleep(1)
                    except:
                        pass
                ...
                logger.info(
                    "You can manually start TensorBoard with: tensorboard --logdir <log_dir>"
                )
...
def compute_vae_loss(
    ...
    try:
        ...
        if torch.rand(1).item() < 0.02:  # Log 2% of batches
            ...
            mean_kl_per_active = (
                kl_per_dim.mean(dim=0)[kl_per_dim.mean(dim=0) > FREE_BITS_PER_DIM].mean().item()
                if active_dims > 0
                else 0
            )
            ...
            if config.epochs > 50 and active_dims < total_dims * 0.5:
                ...
                if torch.rand(1).item() < 0.05:  # Even rarer
                    print(
                        f"ℹ️  Latent Density: {mean_kl_per_dim/FREE_BITS_PER_DIM*100:.1f}% | Avg KL/dim: {mean_kl_per_dim:.3f} (Goal: >{FREE_BITS_PER_DIM})"
                    )
            ...
            if active_dims < total_dims * 0.5 and epoch > 50:
                ...
                print(
                    f"   μ∈[{mu.min():.2f},{mu.max():.2f}], logσ²∈[{logvar.min():.2f},{logvar.max():.2f}]"
                )
                print(f"   Mean KL per active dim: {mean_kl_per_active:.3f}")
            elif kl_raw > 50.0:
                print(
                    f"ℹ️  High KL: {kl_raw:.1f} | Active: {active_dims}/{total_dims} | β={beta_kl:.5f}"
                )
    ...
    # 4. Total loss
    recon_loss = (
        waveform_loss * config.waveform_loss_weight + spectral_loss * config.spectral_loss_weight
    )
...
def calculate_beta(epoch: int, config: TrainingConfig) -> float:
    ...
    if (
        epoch > warmup_epochs
        and hasattr(config, "use_cyclical_annealing")
        and config.use_cyclical_annealing
    ):
        cycle_length = 100
